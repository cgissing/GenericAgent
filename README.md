...
`auth="codex"` reuses `$CODEX_HOME/auth.json` or `~/.codex/auth.json` for credentials, and also reads `$CODEX_HOME/config.toml` or `~/.codex/config.toml` for Codex model-provider routing. If `config.toml` selects `model_provider = "sub2api"` and `[model_providers.sub2api].base_url`, GA uses that base URL unless the GA config explicitly overrides it. If ChatGPT OAuth tokens are present, GA calls the Codex Responses backend; otherwise it falls back to `OPENAI_API_KEY` / `apikey` and the OpenAI Responses API. Official endpoints are defaults only: set `codex_base_url`, `chatgpt_base_url`, `openai_base_url`, `apibase`, `base_url`, or `baseurl` to point GA at a custom Codex/OpenAI gateway for quota management or account switching. A Codex-style gateway such as `https://gateway.example/backend-api/codex` is completed as `https://gateway.example/backend-api/codex/responses`, not forced onto an official host or `/v1` path. GA does not implement its own OAuth flow; run `codex login` when OAuth state is missing.
